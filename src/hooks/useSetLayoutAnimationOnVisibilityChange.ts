...
export function useSetLayoutAnimationOnVisibilityChange(value: any, options?: Options) {
  ...
  if (value !== prevValue.current) {
    ...
    switch (options?.effectType) {
      case 'linear':
        LayoutAnimation.configureNext({...LayoutAnimation.Presets.linear, duration});
        break;
      case 'easeInEaseOut':
        LayoutAnimation.configureNext({...LayoutAnimation.Presets.easeInEaseOut, duration});
        break;
      case 'spring':
        LayoutAnimation.configureNext({...LayoutAnimation.Presets.spring, duration});
        break;
      default: {
        LayoutAnimation.configureNext({
          create: {property: 'opacity', type: 'easeIn', delay: duration * .8},
          duration,
